#############data preparation#####################
rm(list=ls(all=TRUE))
x <- read.table("C:\\Users\\honey66clover\\Desktop\\Weijuan\\Courses\\Statistics\\588 Data Mining\\Dan Yang\\project\\breast cancer\\train.csv",sep=",")
x.test <- read.table("C:\\Users\\honey66clover\\Desktop\\Weijuan\\Courses\\Statistics\\588 Data Mining\\Dan Yang\\project\\breast cancer\\test.csv",sep=",")
x <- as.matrix(x)
...
g<-x[,1]
x<-x[,2:31]
y1<-cbind(rep(1,154),rep(0,154)) #Bs
y2<-cbind(rep(0,146),rep(1,146)) #Ms
y<-rbind(y1,y2)
g.test<-x.test[,1]
x.test<-x.test[,2:31]
y1.test<-cbind(rep(1,203),rep(0,203)) #Bs
y2.test<-cbind(rep(0,66),rep(1,66)) #Ms
...
x.test.pca=x.test%*%x.svd$v #project on the training PC directions!!!!!
...
t=table(g.hat,g)
trainerr=1-(t[1,1]+t[2,2])/n #error rate 0.1333333
