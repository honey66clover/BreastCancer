...
setwd("C:/Users/danyang/Dropbox/588/myself/notes/lecture8")
rm(list=ls())
...
#x[,"famhist"] <- c(0,1)[(x[,"famhist"]=="Present")+1]
x[,"famhist"] <- as.numeric(x[,"famhist"])
x <- as.matrix(x)
g <- x[,"chd"]
y <- cbind(g==1,g==0)*1   # response 
# check all the row sums are 1
all(apply(y,1,sum)==1)
# standardize predictors  and include intercept
# x <- cbind(rep(1,nrow(x)),scale(x[,-10])/sqrt(nrow(x)-1))    # include all of the predictors
x <- cbind(rep(1,nrow(x)),scale(x[,1:2])/sqrt(nrow(x)-1))

B <- solve(t(x)%*%x)%*%t(x)%*%y 
# or equivalently
lm1 <- lm(y[,1]~.,data=data.frame(x[,-1])) #B[,1] first column of B 
lm2 <- lm(y[,2]~.,data=data.frame(x[,-1])) #B[,2] second column of B

# check that B(1,1)+B(1,2)=1
# therefore sum \hat{y}_k = 1
# double check
fitted(lm1)+fitted(lm2) #fitted(lm1) is yhat1 probability to be in group 1

g.hat <- (fitted(lm1)>fitted(lm2))*1 # in group 1
...
plot(x[,2],x[,3],col= c(2,3)[g+1])
abline(b=-1.3787/2.7725,a=(0.6537-0.3463)/2.7725/2,col=4)
...
mu.2.hat <- apply(x[g==0,],2,mean)
# within class covariance sigma.hat
S.w <- (t(x[g==1,] - rep(1,n1)%*% t(mu.1.hat)) %*% (x[g==1,] - rep(1,n1)%*% t(mu.1.hat)) +
        t(x[g==0,] - rep(1,n2)%*% t(mu.2.hat)) %*% (x[g==0,] - rep(1,n2)%*% t(mu.2.hat)))/(n-2)

# decision boundary see p36 in lecture 7
slope <- solve(S.w) %*% (mu.2.hat-mu.1.hat) #beta
intercept <- log(pi2.hat/pi1.hat) - t(mu.2.hat+mu.1.hat) %*% solve(S.w) %*% (mu.2.hat-mu.1.hat)/2 #a
# classification
g.hat <-  ((x %*% slope + as.vector(intercept)) < 0) * 1
table(g.hat,g)
#     g
#g.hat   0   1
#    0 277 116
...
# plot the decision boundary
windows()
plot(x[,1],x[,2],col= c(2,3)[g+1])
abline(b=-slope[1]/slope[2],a=-intercept/slope[2],col=4) #LDA
abline(b=-1.3787/2.7725,a=(0.6537-0.3463)/2.7725/2,col=5) #linear regression

# compare the slope with linear regression
-slope[1]/slope[2]
-1.3787/2.7725
# the same up to rounding error

# compare the intercept with linear regression
-intercept/slope[2]
(0.6537-0.3463)/2.7725/2
# different
